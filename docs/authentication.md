...
- `TRUSTED_PROXY_HEADERS` 可按英文逗号配置可信代理写入客户端 IP 的请求头及优先级，默认值为 `X-Forwarded-For,X-Real-IP`。例如 Cloudflare 入口可配置 `CF-Connecting-IP,X-Forwarded-For,X-Real-IP`。只应填写会被可信代理覆盖或清洗的请求头；应用仍会先校验 TCP 直连来源是否属于 `TRUSTED_PROXIES`，公网直连请求不能仅靠伪造该请求头覆盖 IP。

...
func recordManageAuditFor(c *gin.Context, targetUserId int, action string, params map[string]interface{}) {
	...
	model.RecordOperationAuditLog(operatorUserId, auditContentEN(action, params), common.RequestClientIP(c), action, params, auditOperatorInfo(c), nil)
	markAuditLogged(c)
}
...
func recordUserSecurityAudit(c *gin.Context, userId int, action string, params map[string]interface{}) {
	model.RecordOperationAuditLog(userId, auditContentEN(action, params), common.RequestClientIP(c), action, params, nil, nil)
}
